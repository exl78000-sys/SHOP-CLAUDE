...
### 2026-07-26（下）
- 工資試算頁新增批次計算：經理／褲子師傅可切換「單筆／批次」，批次一次輸入各品項件數直接算總工資
  （經理：二件式/三件式/外套/褲子/背心/襯衫；褲子師傅：褲子/背心，皆固定1900）；外套師傅不變

### 2026-07-26
- CLAUDE.md 工資章節更新為完整版：補上扣眼精確算法（單排=排扣扣數欄位/雙排固定3顆＋袖口扣數×2）、
  背心工資分流（褲子師傅1900／外套師傅單排2200雙排2500）、經理費含背心項；
  新增「工資試算頁」完整說明段落（三師傅分頁、各自可選項目、獨立結算邏輯）
